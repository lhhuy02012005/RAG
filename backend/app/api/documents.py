...
async def stream_workspace_document_events(
    workspace_id: int,
    db: AsyncSession = Depends(get_db),
):
    """SSE stream for document status updates in a workspace."""
    result = await db.execute(select(KnowledgeBase.id).where(KnowledgeBase.id == workspace_id))
    kb_id = result.scalar_one_or_none()
    if kb_id is None:
        raise NotFoundError("KnowledgeBase", workspace_id)

    async def event_generator():
        queue = await subscribe_workspace_events(workspace_id)
        try:
            yield ": connected\n\n"
            while True:
                try:
                    payload = await asyncio.wait_for(queue.get(), timeout=SSE_HEARTBEAT_INTERVAL)
                    status_name = str(payload.get("status", "status"))
                    yield format_sse_event(status_name, payload)
                except asyncio.TimeoutError:
                    yield ": heartbeat\n\n"
        except asyncio.CancelledError:
            raise
        finally:
            await unsubscribe_workspace_events(workspace_id, queue)
