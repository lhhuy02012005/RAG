    ...
    async def query(
        ...
        if kg_task:
            ...
            try:
                ...
                kg_elapsed_ms = int((time.perf_counter() - kg_t0) * 1000)
        ...
        try:
            raw_chunks, raw_citations = await asyncio.wait_for(
                vector_task,
                timeout=settings.NEXUSRAG_VECTOR_QUERY_TIMEOUT,
            )
        except asyncio.TimeoutError:
            vector_timed_out = True
            vector_task.cancel()
            logger.warning(
                f"Vector retrieval timed out after "
                f"{settings.NEXUSRAG_VECTOR_QUERY_TIMEOUT}s "
                f"for workspace {self.workspace_id}. Returning KG-only context."
            )
        except Exception as e:
            logger.warning(f"Vector retrieval failed, continuing with KG only: {e}")
        vector_elapsed_ms = int((time.perf_counter() - vector_t0) * 1000)
        ...
        if raw_chunks:
            try:
                chunks, citations = await asyncio.wait_for(
                    asyncio.to_thread(
                        self._rerank_chunks, question, raw_chunks, raw_citations, top_k
                    ),
                    timeout=settings.NEXUSRAG_RERANK_TIMEOUT,
                )
            except asyncio.TimeoutError:
                rerank_timed_out = True
                logger.warning(
                    f"Rerank timed out after {settings.NEXUSRAG_RERANK_TIMEOUT}s "
                    f"for workspace {self.workspace_id}. Falling back to vector top-{top_k}."
                )
                chunks = raw_chunks[:top_k]
                citations = raw_citations[:top_k]
            except Exception as e:
            ...
            chunks, citations = [], []
        rerank_elapsed_ms = int((time.perf_counter() - rerank_t0) * 1000)
        ...
        media_elapsed_ms = int((time.perf_counter() - media_t0) * 1000)
        ...
        logger.info(
            "Deep retrieval timings | workspace=%s | mode=%s | total=%sms | "
            "kg=%sms | vector=%sms | rerank=%sms | media=%sms | "
            "vector_timeout=%s | rerank_timeout=%s | "
            "raw_chunks=%s | final_chunks=%s",
            self.workspace_id,
            mode,
            total_elapsed_ms,
            kg_elapsed_ms,
