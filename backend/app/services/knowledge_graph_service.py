...
class KnowledgeGraphService:
    ...
    async def _get_rag(self):
        ...
        self._rag = LightRAG(
            ...
            # Ghi đè trực tiếp timeout của LLM và Embedding thông qua các tham số hợp lệ
            default_llm_timeout=900,        # Tăng lên 15 phút
            default_embedding_timeout=600,  # Tăng lên 10 phút
            addon_params={
                "language": self.kg_language,
                "entity_types": self.kg_entity_types,
                "embedding_batch_num": settings.NEXUSRAG_KG_EMBEDDING_BATCH_NUM,     # Giới hạn số luồng Embedding song song (mặc định là 8 hoặc 16)
                "entity_extraction_limit": settings.NEXUSRAG_KG_ENTITY_EXTRACTION_LIMIT, # Giới hạn số thực thể tối đa trích xuất mỗi lần để giảm tải LLM
                "llm_model_max_token_size": settings.NEXUSRAG_KG_LLM_MAX_TOKEN_SIZE, # Giới hạn ngữ cảnh để tránh tràn RAM
            },
        )
            ...
            try:
                ...
                if not all_nodes:
                    ...
                    logger.warning(
                        ...
                        f"Model '{settings.OLLAMA_MODEL}' may not support LightRAG's entity extraction format. "
                        f"Consider using a larger model (qwen2.5:3b) for KG."
                    )
            except Exception:
                pass
