...
class DeepDocumentParser:
    ...
    def _get_converter(self):
        ...
        pipeline_options.generate_picture_images = settings.RAG_ENABLE_IMAGE_EXTRACTION
        pipeline_options.images_scale = settings.RAG_DOCLING_IMAGES_SCALE
        pipeline_options.do_formula_enrichment = settings.RAG_ENABLE_FORMULA_ENRICHMENT
    ...
    def _parse_with_docling(
        ...
        if settings.RAG_ENABLE_TABLE_CAPTIONING and tables:
    ...
    def _extract_images_with_urls(
        ...
        """
        if not settings.RAG_ENABLE_IMAGE_EXTRACTION:
            return [], []
        ...
        for pic in doc.pictures:
            if picture_count >= settings.RAG_MAX_IMAGES_PER_DOC:
        ...
        if settings.RAG_ENABLE_IMAGE_CAPTIONING and images:
    ...
    def _caption_tables(self, tables: list[ExtractedTable]) -> None:
        ...
        for tbl in tables:
            ...
            try:
                ...
                if len(table_md) > settings.RAG_MAX_TABLE_MARKDOWN_CHARS:
                    table_md = table_md[:settings.RAG_MAX_TABLE_MARKDOWN_CHARS] + "\n... (truncated)"
